...
const PostTitles = ({ posts, type }) => {
    ...
    try {
      if (type === "comment") {
        ...
        const response = await axios.delete(
          `${url}/DeleteComment?commentId=${postId}`
        );
        ...
        const response = await axios.delete(
          `${url}/DeletePostAndComments?postId=${postId}`
        );
  ...
  if (type === "posts") {
    ...
    sortedPosts = posts
      .slice()
      .sort((a, b) => new Date(b.commentSeq) - new Date(a.commentSeq));
  } else {
    sortedPosts = [posts];
  }
          ...
          <div className="col position-relative">
            <a
              ...
              className={
                Array.isArray(posts)
                  ...
                  : "card text-decoration-none isDisabled"
              }
            ...
            {post.creatorId === id && (
              <div
                ...
                style={{ right: "20px", top: "-15px", zIndex: "1" }}
              >
                <button
